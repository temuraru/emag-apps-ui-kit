<script type="text/javascript">
    var sidebar = {
        menuItems: [
            {
                'title': 'Start',
                'href': 'javascript:void(0)',
                'data-href': '/emag-apps-ui-kit/demo/index.php',
                'icon': 'fa-flag-checkered',
                'keywords': '',
                'children': [
                    {
                        'title': 'Overview',
                        'href': '/emag-apps-ui-kit/demo/index.php#overview',
                        'keywords': 'abc'
                    },
                    {
                        'title': 'Getting started',
                        'href': '/emag-apps-ui-kit/demo/index.php#getting-started',
                        'keywords': ''
                    }
                ]
            },
            {
                'title': 'Layout',
                'href': 'javascript:void(0)',
                'data-href': '/emag-apps-ui-kit/demo/layout.php',
                'icon': 'fa-columns',
                'keywords': '',
                'children': [
                    {
                        'title': 'Boilerplate',
                        'href': '/emag-apps-ui-kit/demo/layout.php'
                    },
                    {
                        'title': 'Top Navigation',
                        'href': '/emag-apps-ui-kit/demo/layout.php#top-navigation'
                    },
                    {
                        'title': 'Sidebar',
                        'href': '/emag-apps-ui-kit/demo/layout.php#main-sidebar'
                    },
                    {
                        'title': 'Footer',
                        'href': '/emag-apps-ui-kit/demo/layout.php#main-footer'
                    },
                    {
                        'title': 'Grid Layout',
                        'href': '/emag-apps-ui-kit/demo/layout.php#grid-layout'
                    },
                    {
                        'title': 'Helper classes',
                        'href': '/emag-apps-ui-kit/demo/helper_classes.php'
                    }
                ]
            },
            {
                'title': 'Branding',
                'href': 'javascript:void(0)',
                'data-href': '/emag-apps-ui-kit/demo/branding.php',
                'icon': 'fa-paint-brush',
                'keywords': '',
                'children': [
                    {
                        'title': 'Color scheme',
                        'href': '/emag-apps-ui-kit/demo/branding.php'
                    },
                    {
                        'title': 'Navigation themes',
                        'href': '/emag-apps-ui-kit/demo/branding.php#navigationThemes'
                    },
                    {
                        'title': 'Typography',
                        'href': 'javascript:void(0)',
                        'children': [
                            {
                                'title': 'Typefaces',
                                'href': '/emag-apps-ui-kit/demo/typography.php#typefaces'
                            },
                            {
                                'title': 'Headings and paragraphs',
                                'href': '/emag-apps-ui-kit/demo/typography.php#headings-and-paragraphs'
                            },
                            {
                                'title': 'Utility tags',
                                'href': '/emag-apps-ui-kit/demo/typography.php#utility-tags'
                            },
                            {
                                'title': 'Alignment classes',
                                'href': '/emag-apps-ui-kit/demo/typography.php#alignment-classes'
                            },
                            {
                                'title': 'Address example',
                                'href': '/emag-apps-ui-kit/demo/typography.php#address-example'
                            },
                            {
                                'title': 'Contextual classes',
                                'href': '/emag-apps-ui-kit/demo/typography.php#contextual-classes'
                            },
                            {
                                'title': 'Text case and capitalization',
                                'href': '/emag-apps-ui-kit/demo/typography.php#text-case-and-capitalization'
                            },
                            {
                                'title': 'Block quotes',
                                'href': '/emag-apps-ui-kit/demo/typography.php#block-quotes'
                            },
                            {
                                'title': 'Print',
                                'href': '/emag-apps-ui-kit/demo/typography.php#print'
                            },
                            {
                                'title': 'Simple preformatted block',
                                'href': '/emag-apps-ui-kit/demo/typography.php#simple-preformatted-block'
                            },
                            {
                                'title': 'Variables',
                                'href': '/emag-apps-ui-kit/demo/typography.php#variables'
                            },
                            {
                                'title': 'Sample output',
                                'href': '/emag-apps-ui-kit/demo/typography.php#sample-output'
                            },
                            {
                                'title': 'Descriptions',
                                'href': '/emag-apps-ui-kit/demo/typography.php#descriptions'
                            },
                            {
                                'title': 'Horizontal descriptions',
                                'href': '/emag-apps-ui-kit/demo/typography.php#horizontal-descriptions'
                            },
                            {
                                'title': 'Unordered lists',
                                'href': '/emag-apps-ui-kit/demo/typography.php#unordered-list'
                            }
                        ]
                    },
                    {
                        'title': 'Icons & flags',
                        'href': '/emag-apps-ui-kit/demo/branding.php#icons'
                    }
                ]
            },
            {
                'title': 'CSS Components',
                'href': 'javascript:void(0)',
                'data-href': '/emag-apps-ui-kit/demo/css_components.php',
                'icon': 'fa-archive',
                'keywords': '',
                'children': [
                    {
                        'title': 'Tables',
                        'href': '/emag-apps-ui-kit/demo/css_components.php#css-tables'
                    },
                    {
                        'title': 'Forms',
                        'href': '/emag-apps-ui-kit/demo/css_components.php#basic-forms'
                    },
                    {
                        'title': 'Buttons',
                        'href': '/emag-apps-ui-kit/demo/css_components.php#buttons'
                    },
                    {
                        'title': 'CSS Flipswitch',
                        'href': '/emag-apps-ui-kit/demo/css_components.php#css-flipswitch'
                    },
                    {
                        'title': 'Labels and badges',
                        'href': '/emag-apps-ui-kit/demo/css_components.php#css-labels-and-badges'
                    },
                    {
                        'title': 'Alerts and notifications',
                        'href': '/emag-apps-ui-kit/demo/css_components.php#css-alerts-and-notifications'
                    },
                    {
                        'title': 'Progress Bars',
                        'href': '/emag-apps-ui-kit/demo/css_components.php#css-progressbars'
                    },
                    {
                        'title': 'Panels',
                        'href': '/emag-apps-ui-kit/demo/css_components.php#css-panels'
                    },
                    {
                        'title': 'Page header',
                        'href': '/emag-apps-ui-kit/demo/css_components.php#css-page-header'
                    },
                    {
                        'title': 'Embeds',
                        'href': '/emag-apps-ui-kit/demo/css_components.php#css-embeds'
                    }
                ]
            },
            {
                'title': 'JS Components',
                'href': 'javascript:void(0)',
                'data-href': '/emag-apps-ui-kit/demo/js_components.php',
                'icon': 'fa-code',
                'keywords': '',
                'children': [
                    {
                        'title': 'Complex form elements',
                        'href': 'javascript:void(0)',
                        'children': [
                            {
                                'title': 'Masked inputs',
                                'href': '/emag-apps-ui-kit/demo/js_complex_form_elements.php'
                            },
                            {
                                'title': 'Textarea with input limiter',
                                'href': '/emag-apps-ui-kit/demo/js_complex_form_elements.php#jsTextareaWithInputLimiter'
                            },
                            {
                                'title': 'File upload',
                                'href': '/emag-apps-ui-kit/demo/js_complex_form_elements.php#jsFileUpload'
                            },
                            {
                                'title': 'Selectize tag input',
                                'href': '/emag-apps-ui-kit/demo/js_complex_form_elements.php#jsSelectizeTagInput'
                            },
                            {
                                'title': 'jQuery UI autocomplete',
                                'href': '/emag-apps-ui-kit/demo/js_complex_form_elements.php#jsJQueryUIAutocomplete'
                            },
                            {
                                'title': 'jQuery UI sliders',
                                'href': '/emag-apps-ui-kit/demo/js_complex_form_elements.php#jsJQueryUISliders'
                            },
                            {
                                'title': 'jQuery UI numeric input (Spinner)',
                                'href': '/emag-apps-ui-kit/demo/js_complex_form_elements.php#jsJQueryNumericInput'
                            },
                            {
                                'title': 'Chosen.js autocomplete',
                                'href': '/emag-apps-ui-kit/demo/js_complex_form_elements.php#jsChosenJsAutocomplete'
                            }
                        ]
                    },
                    {
                        'title': 'jQGrid',
                        'href': 'javascript:void(0)',
                        'children': [
                            {
                                'title': 'Basic',
                                'href': '/emag-apps-ui-kit/demo/plugin_jqgrid.php'
                            },
                            {
                                'title': 'Add new row programatically',
                                'href': '/emag-apps-ui-kit/demo/plugin_jqgrid.php#jqGrid-add-new-row-programatically'
                            },
                            {
                                'title': 'With sticky panels',
                                'href': '/emag-apps-ui-kit/demo/plugin_jqgrid.php#jqGrid-with-sticky-panels'
                            },
                            {
                                'title': 'With column chooser',
                                'href': '/emag-apps-ui-kit/demo/plugin_jqgrid.php#jqGrid-with-column-chooser'
                            },
                            {
                                'title': 'With selectable rows',
                                'href': '/emag-apps-ui-kit/demo/plugin_jqgrid.php#jqGrid-with-selectable-rows'
                            },
                            {
                                'title': 'With hierarchical elements (Tree)',
                                'href': '/emag-apps-ui-kit/demo/plugin_jqgrid.php#jqGrid-hierarchical-elements-tree'
                            },
                            {
                                'title': 'With subgrid',
                                'href': '/emag-apps-ui-kit/demo/plugin_jqgrid.php#jqGrid-with-subgrid'
                            },
                            {
                                'title': 'With inline editing',
                                'href': '/emag-apps-ui-kit/demo/plugin_jqgrid.php#jqGrid-with-inline-editing'
                            },
                            {
                                'title': 'With actions',
                                'href': '/emag-apps-ui-kit/demo/plugin_jqgrid.php#jqGrid-with-actions'
                            },
                            {
                                'title': 'With no data',
                                'href': '/emag-apps-ui-kit/demo/plugin_jqgrid.php#jqGrid-with-no-data'
                            },
                            {
                                'title': 'With data formatters',
                                'href': '/emag-apps-ui-kit/demo/plugin_jqgrid.php#jqGrid-with-data-formatters'
                            },
                            {
                                'title': 'With lot of columns and data formatters',
                                'href': '/emag-apps-ui-kit/demo/plugin_jqgrid.php#jqGrid-with-lot-of-columns-and-data-formatters'
                            }
                        ]
                    },
                    {
                        'title': 'Modals',
                        'href': '/emag-apps-ui-kit/demo/js_components.php'
                    },
                    {
                        'title': 'Custom notifications (Growl)',
                        'href': '/emag-apps-ui-kit/demo/js_components.php#js-custom-notifications'
                    },
                    {
                        'title': 'Tooltips',
                        'href': '/emag-apps-ui-kit/demo/js_components.php#js-tooltips'
                    },
                    {
                        'title': 'Popovers',
                        'href': '/emag-apps-ui-kit/demo/js_components.php#js-popovers'
                    },
                    {
                        'title': 'Tabs',
                        'href': '/emag-apps-ui-kit/demo/js_components.php#js-tabs'
                    },
                    {
                        'title': 'Utilities',
                        'href': '/emag-apps-ui-kit/demo/js_components.php#js-utilities'
                    },
                    {
                        'title': 'Block UI',
                        'href': '/emag-apps-ui-kit/demo/js_components.php#js-block-ui'
                    },
                    {
                        'title': 'Date & Time',
                        'href': '/emag-apps-ui-kit/demo/plugin_datetime.php'
                    },
                    {
                        'title': 'Data formatters',
                        'href': '/emag-apps-ui-kit/demo/plugin_data_formatters.php'
                    },
                    {
                        'title': 'Fancytree',
                        'href': '/emag-apps-ui-kit/demo/plugin_fancytree.php'
                    },
                    {
                        'title': 'Hierarchical select',
                        'href': '/emag-apps-ui-kit/demo/plugin_hierarchicalselect.php'
                    },
                    {
                        'title': 'IntroJS',
                        'href': '/emag-apps-ui-kit/demo/plugin_introjs.php'
                    },
                    {
                        'title': 'Image gallery',
                        'href': '/emag-apps-ui-kit/demo/image_gallery.php'
                    },
                    {
                        'title': 'Wysiwyg editor',
                        'href': '/emag-apps-ui-kit/demo/plugin_wysiwyg.php'
                    },
                    {
                        'title': 'Sortable',
                        'href': '/emag-apps-ui-kit/demo/sortable.php'
                    },
                    {
                        'title': 'Stats',
                        'href': 'javascript:void(0)',
                        'children': [
                            {
                                'title': 'Value box',
                                'href': '/emag-apps-ui-kit/demo/plugin_stats.php'
                            },
                            {
                                'title': 'Charts',
                                'href': '/emag-apps-ui-kit/demo/plugin_stats.php#charts'
                            }
                        ]
                    }
                ]
            },
            {
                'title': 'Useful Pages',
                'href': 'javascript:void(0)',
                'data-href': '',
                'icon': 'fa-star',
                'keywords': '',
                'children': [
                    {
                        'title': 'Log In & Sign Up',
                        'href': '/emag-apps-ui-kit/demo/authentication.php'
                    },
                    {
                        'title': 'Error pages',
                        'href': 'javascript:void(0)',
                        'children': [
                            {
                                'title': '404',
                                'href': '/emag-apps-ui-kit/demo/err_404.php'
                            },
                            {
                                'title': '503',
                                'href': '/emag-apps-ui-kit/demo/err_503.php'
                            },
                            {
                                'title': 'Logged out',
                                'href': '/emag-apps-ui-kit/demo/err_logged_out.php'
                            }
                        ]
                    },
                    {
                        'title': 'Add form example',
                        'href': '/emag-apps-ui-kit/demo/add_form_example.php'
                    },
                    {
                        'title': 'Blank page',
                        'href': '/emag-apps-ui-kit/demo/blank_page.php'
                    },
                    {
                        'title': 'Email template',
                        'href': '/emag-apps-ui-kit/demo/email_template.php'
                    },
                    {
                        'title': 'Wizard',
                        'href': '/emag-apps-ui-kit/demo/plugin_wizard.php'
                    }
                ]
            },
            {
                'title': 'Technical details',
                'href': 'javascript:void(0)',
                'data-href': '',
                'icon': 'fa-cog',
                'children': [
                    {
                        'title': 'Plugins versions',
                        'href': '/emag-apps-ui-kit/demo/plugins_versions.php'
                    }
                ]
            }
        ]
    };

    createSidebar(sidebar);
</script>